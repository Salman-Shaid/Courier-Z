...
const SignUp = () => {
  ...
  const handleSubmit = async event => {
    ...
    const role = form.role.value  // Get the role from the form
    console.log(role);
    // 1. Send image data to image bb (your existing logic)
    ...
    try {
      ...
      // 3. Save username, profile photo, and role
      await updateUserProfile(name, photoUrl)

      // Send user role to the backend
      await axios.post(`${import.meta.env.VITE_API_URL}/users/${email}`, {
        name: name,
        image: photoUrl,
        email: email,
        role: role,  // Send the role dynamically
        phone: phone,
      })
  ...
  return (
    <div className='flex justify-center items-center min-h-screen bg-white bg-cover bg-center'
      style={{
